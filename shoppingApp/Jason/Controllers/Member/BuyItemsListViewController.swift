    ...
    private func loadDataFromServer(){
        ...
        URLSession.shared.dataTask(with: request) { [weak self] (getOrderData, response, error) in
            ...
            if let getOrderData = getOrderData {
                ...
                self?.products = items.sorted(by: { (item1, item2) -> Bool in
...
extension BuyItemsListViewController: UICollectionViewDataSource, UICollectionViewDelegate{
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if let productVC = UIStoryboard(name: "Search", bundle: nil).instantiateViewController(withIdentifier: "ProductViewController") as? ProductViewController {
            ...
            // 開啟商品畫面
            productVC.selectedProduct = products[indexPath.row]
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if !products.isEmpty{
            ...
            cell.priceLabel.text = products[indexPath.row].price.description
            cell.pictureImageView.image = nil
            // 設定cell裡面buttons的tagID
            cell.isLikeButton.tag = indexPath.row
            cell.cartButton.tag = indexPath.row
            if UserInfo.cartList.contains(products[indexPath.row].item_id){
                cell.cartButton.imageView?.image = UIImage(systemName: "cart.fill")
            } else {
                cell.cartButton.imageView?.image = UIImage(systemName: "cart")
            }
            
            if UserInfo.favoriteList.contains(products[indexPath.row].item_id){
            ...
            let url = products[indexPath.row].media_info!
